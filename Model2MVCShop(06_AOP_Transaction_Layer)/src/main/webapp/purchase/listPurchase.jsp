	...
	<tr class="ct_list_pop">
		<td align="center">
			...
			<c:choose>
				<c:when test = "${ fn:trim(i.tranCode) eq '' }" >
					<a href="/getPurchase.do?prodNo=${i.purchaseProd.prodNo}">${i.purchaseProd.prodNo}</a>
				</c:when>
				<c:when test = "${ fn:trim(i.tranCode) eq '1' }" >
					<a href="/getPurchase.do?prodNo=${i.purchaseProd.prodNo}">${i.purchaseProd.prodNo}</a>
				</c:when>
				<c:otherwise>
					${i.purchaseProd.prodNo}
				</c:otherwise>
			</c:choose>
		...
		<td align="left">
			...
			<c:choose>
				...
				<c:when test = "${ fn:trim(i.tranCode) eq '2' }" >
					<a href="/updateTranCode.do?tranNo=${i.tranNo}&tranCode=3">물건도착</a> 
				</c:when>
				<c:otherwise>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top: 10px;">
	<tr>
		<td align="center">
		 	...
		 	<c:if test="${ resultPage.page > resultPage.pageUnit }">
					<a href="/listPurchase.do?page=${resultPage.endUnitPage - resultPage.pageUnit}"><< 이전 &nbsp</a>
			</c:if>
			...
			<c:if test="${ resultPage.endUnitPage < resultPage.maxPage }">
					<a href="/listPurchase.do?page=${resultPage.endUnitPage+1}"> &nbsp 이후 >></a>
			</c:if>
